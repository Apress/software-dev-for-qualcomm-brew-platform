...
/**
 * @name DBGPRINT_HeapUsed
 * @memo Prints amount of help used to log.
 * @doc Prints the amount of heap used to the debug console.
 */
#ifdef _DEBUG
#define DBGPRINT_HeapUsed( pThis ) \
{\
  IHeap *pIHeap;\
  ...
  if ( pIHeap )\
  {\
    uint32 nMemoryUsed = IHEAP_GetMemStats(pIHeap);\
    DBGPRINTF("%s(%d) : %d bytes used", __FILE__, __LINE__, nMemoryUsed );\
