...
{ 
	...
	result = pThis->m_app.m_apControl[ Ctl_NavMenu ] && 
				...
				pThis->m_app.m_apControl[ Ctl_Static ] ? 
				SUCCESS : 
				EFAILED;
...
{ 
	...
	// Set up the application's state machine
	pThis->m_app.m_pState = MALLOC( sizeof( CState ) );
	result = pThis->m_app.m_pState ? SUCCESS : EFAILED;

	// Set up the application's preferences
	if ( result == SUCCESS )
	{
		pThis->m_app.m_pAppPrefs = MALLOC( sizeof( CAppPrefs ) );
		if ( !GetAppPrefs( pThis ) )
		{
			FREE( pThis->m_app.m_pState );
			result = EFAILED;
		}
		else 
		if ( ISHELL_GetPrefs( GetShell( pThis ), AEECLSID_OURS,
			                  APP_PREFS_VERSION, 
			                  GetAppPrefs( pThis ), 
			                  sizeof(CAppPrefs) ) != SUCCESS )
		{
			AS_DefaultPrefs( GetAppPrefs( pThis ) );
			if ( ISHELL_SetPrefs( GetShell( pThis ), 
								AEECLSID_OURS,
								APP_PREFS_VERSION, 
								GetAppPrefs( pThis ), 
								sizeof(CAppPrefs) ) != SUCCESS )
...
static void Main_Free( IApplet *p )
{
	...
	// Commit the application preferences
	ISHELL_SetPrefs( GetShell( pThis ), 
					AEECLSID_OURS,
					APP_PREFS_VERSION, 
					GetAppPrefs( pThis ), 
					sizeof(CAppPrefs) );
	
	// Release the application preferences
	FREE( GetAppPrefs( pThis ) );
								...
								uint32 dwParam )
{
	...
	{
		case EVT_NOTIFY:
			result = AS_HandleNotify( pThis, wParam, dwParam );
			break;
		...
		case EVT_COPYRIGHT_END:
			...
			if ( Main_Start( pThis ) == SUCCESS ) 
			{		
				State_Push( pThis, AS_FIRSTSTATE );
			}
			else
			{
				ISHELL_CloseApplet( GetShell( pThis ), FALSE );
			}
			result = TRUE;
			break;
