...
/**
 * @name _NumericNode
 * @memo Singly linked list of numeric nodes.
 * @doc Provides the implementation for a simple singly-linked list of numeric nodes.
 * The head of the list is denoted with the data fields set to 0.
 */
typedef struct _NumericNode
{
	/// Next node
	struct _NumericNode *m_pNext;
	/// data in this node
	uint32 m_nData;
	/// metadata in this node.
	uint32 m_nMetaData;
} NumericNode;
 ...
 * @return nothing
 */
#define NodeLinkSorted( pThis, head, n, fnSort, kind ) \
{ \
	kind *_p = head; \
	while( _p->m_pNext && fnSort( pThis, n, _p ) > 0 ) _p = _p->m_pNext; \
	n->m_pNext = _p->m_pNext; \
	_p->m_pNext = n; \
}
