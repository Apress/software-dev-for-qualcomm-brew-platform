...
int AS_Init( CAppPtr pThis )
{
  ...
  if ( pAppData )
  {
	...
	result = ISHELL_CreateInstance( GetShell( pThis ), AEECLSID_THEEXTENSION, (void **)&pAppData->pIExtension );
	if ( result == SUCCESS )
		DBGPRINTF("Creation OK");
	else
		DBGPRINTF("Creation FAILED %d", result );
...
void AS_Free( CAppPtr pThis )
{
  ...
  if ( pAppData )
  {
	if ( pAppData->pIExtension )
	{
		IEXTENSION_Release( pAppData->pIExtension );
		pAppData->pIExtension = NULL;
	}
	FREE( pAppData );
	pAppData = NULL;
  }
